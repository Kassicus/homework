        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if current_user.is_authenticated:
                    logger.info(f"User {current_user.username} performed {activity_type}")
                
                result = f(*args, **kwargs)
                
                # Log successful completion
                if current_user.is_authenticated:
                    logger.info(f"User {current_user.username} completed {activity_type} successfully")
                ...
                # Log error
                if current_user.is_authenticated:
                    logger.error(f"User {current_user.username} failed {activity_type}: {e}")
                raise
        ...
        if 'contract_file' in request.files:
            ...
            if file and file.filename:
                ...
                if not validate_file_extension(file.filename):
                    flash('Invalid file type. Please upload a PDF, DOCX, DOC, TXT, or RTF file.', 'error')
                ...
                if not validate_file_size(file_size):
                    ...
                    flash(f'File too large. Maximum size is {format_file_size(max_size)}.', 'error')
                    return redirect(request.url)
