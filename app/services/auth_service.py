...
class AuthService:
    ...
    def login_user_service(user, remember=False):
        ...
        try:
            ...
            logger.info(f"User logged in: {user.username} from IP: {request.remote_addr}")
    ...
    def update_user_status(user_id, is_active, updated_by):
        ...
        try:
            ...
            logger.info(f"User status updated by {updated_by.username}: {user.username} - {old_status} -> {is_active}")
            return True
