...
class ContractService:
    ...
    def soft_delete_contract(contract_id, deleted_by):
        ...
        try:
            ...
            logger.info(
                f"Contract soft deleted: {contract.title} by user {deleted_by.username}"
            )
            return contract
    ...
    def restore_contract(contract_id, restored_by):
        ...
        try:
            ...
            logger.info(
                f"Contract restored: {contract.title} by user {restored_by.username}"
            )
            return contract
    ...
    def get_contract_statistics():
        ...
        try:
            total_contracts = Contract.query.filter(
                Contract.deleted_at.is_(None)
            ).count()
            ...
            total_value = (
                db.session.query(db.func.sum(Contract.contract_value))
                .filter(
                    and_(
                        Contract.deleted_at.is_(None),
    ...
    def cleanup_expired_deleted_contracts():
        ...
        try:
            ...
            expired_contracts = Contract.query.filter(
                and_(
                    Contract.deleted_at <= cutoff_date, Contract.deleted_at.isnot(None)
                )
            ).all()
            ...
            for contract in expired_contracts:
                try:
                    ...
                    logger.info(
                        f"Expired deleted contract permanently removed: {contract.title}"
                    )

                except Exception as e:
                    logger.error(
                        f"Error removing expired deleted contract {contract.id}: {e}"
                    )

            db.session.commit()
            logger.info(
                f"Cleanup completed: {deleted_count} expired deleted contracts removed"
            )
