...
class FileService:
    ...
    def save_uploaded_file(file, contract_id=None):
        ...
        try:
            ...
            if not FileService.allowed_file(file.filename):
                raise ValueError(f"File type not allowed. Allowed types: {', '.join(FileService.ALLOWED_EXTENSIONS)}")
    ...
    def extract_text_from_file(file_path, mime_type):
        ...
        try:
            ...
            logger.info(f"Text extracted successfully from {file_path}: {len(text)} characters")
            return text
    ...
    def move_to_deleted_folder(file_path, filename):
        ...
        try:
            ...
            logger.info(f"File moved to deleted folder: {filename} -> {deleted_filename}")
            ...
            for filename in os.listdir(deleted_path):
                ...
                if file_time < cutoff_date:
                    try:
                        ...
                        logger.error(f"Error removing expired deleted file {filename}: {e}")
