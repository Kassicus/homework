...
class Contract(db.Model):
    ...
    access_history = db.relationship('ContractAccessHistory', backref='contract', lazy='dynamic',
    ...
    VALID_STATUSES = [
        STATUS_DRAFT, STATUS_UNDER_REVIEW, STATUS_ACTIVE, 
        STATUS_EXPIRED, STATUS_TERMINATED, STATUS_RENEWED
    ]
    ...
    def restore(self, restored_by):
        ...
        self.update_status(self.STATUS_DRAFT, restored_by, "Contract restored from deletion")
...
class ContractAccessHistory(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    contract_id = db.Column(db.Integer, db.ForeignKey('contracts.id'), nullable=False, index=True)
    accessed_by = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, index=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'accessed_by_username': self.accessed_by_user.username if self.accessed_by_user else None,
