    ...
    try:
        ...
        for status in Contract.VALID_STATUSES:
            count = Contract.query.filter(
                Contract.status == status, Contract.deleted_at.is_(None)
            ).count()
        ...
        for i in range(12):
            ...
            count = Contract.query.filter(
                func.date(Contract.created_at).between(
        ...
        top_clients = (
            db.session.query(
                Client.name, func.count(Contract.id).label("contract_count")
            )
            .join(Contract)
        ...
        return render_template(
            "dashboard/contracts.html", contracts_data=contracts_data
        )
...
def reports():
    ...
    try:
        ...
        value_summary = (
            ...
            .filter(Contract.deleted_at.is_(None), Contract.contract_value.isnot(None))
            .first()
        )
        ...
        contracts_by_type = (
            db.session.query(
                Contract.contract_type, func.count(Contract.id).label("count")
            )
            .filter(Contract.deleted_at.is_(None))
...
def activity():
    ...
    try:
        ...
        access_logs = (
            ContractAccessHistory.query.order_by(
...
def notifications():
    ...
    try:
        ...
        return render_template(
            "dashboard/notifications.html", notifications_data=notifications_data
        )
...
def api_expiring():
    ...
    try:
        ...
        return jsonify({"expiring_contracts": results, "days": days})
