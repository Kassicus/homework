...
def new():
    ...
    if request.method == 'POST':
        try:
            ...
            current_app.logger.info(f"New client created: {client.name} by user {current_user.username}")
...
def edit(client_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            current_app.logger.info(f"Client updated: {client.name} by user {current_user.username}")
...
def delete(client_id):
    ...
    try:
        ...
        if client.contracts.count() > 0:
            flash(f'Cannot delete client "{client.name}" because they have {client.contracts.count()} contract(s).', 'error')
            return redirect(url_for('clients.show', client_id=client.id))
        ...
        current_app.logger.info(f"Client deleted: {client_name} by user {current_user.username}")
...
def search():
    ...
    try:
        ...
        return render_template('clients/search.html', clients=clients, search_term=search_term)
        
    except Exception as e:
        logger.error(f"Error searching clients: {e}")
        flash('An error occurred while searching clients.', 'error')
        return render_template('clients/search.html', clients=[], search_term=search_term)
